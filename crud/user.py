...
async def get_users_by_role(session: AsyncSession, role: UserRolesEnum) -> list[User]:
    result = await session.execute(select(User).filter(User.role == role))
    return list(result.scalars().all())


async def get_admins(session: AsyncSession) -> list[User]:
    return await get_users_by_role(session, UserRolesEnum.ADMIN)


async def get_dispatchers(session: AsyncSession) -> list[User]:
    return await get_users_by_role(session, UserRolesEnum.DISPATCHER)
